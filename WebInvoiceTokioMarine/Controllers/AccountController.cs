...
namespace WebInvoiceTokioMarine.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var user = _db.Users.FirstOrDefault(a => a.UserName == model.UserName || a.Email == model.UserName );
            if (user == null)
            {
        ...
        public async Task<ActionResult> RegisterSupplier(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var supplierUser = new Supplier { UserName = model.UserName, Email = model.Email };
                var result = await UserManager.CreateAsync(supplierUser, model.Password);
                if (result.Succeeded)
                {
                    await SignInManager.SignInAsync(supplierUser, isPersistent:false, rememberBrowser:false);
        ...
        public async Task<ActionResult> RegisterCustomer(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var customerUser = new Customer { UserName = model.UserName, Email = model.Email };
